...
function setupTriangles() {
    ...
    for (i=0; i<n; i++) {
    var basepoint0x=Math.random()*4.0/n; var basepoint0y=Math.random()*2.5/n;
    var basepoint1x=Math.random()*3.0/n; var basepoint1y=1.0/n;
    var basepoint2x=Math.random()*5.0/n; var basepoint2y=Math.random()*2.5/n;
	...
	var basepoint5x=Math.random()*4.0/n; var basepoint5y=Math.random()*2.5/n;
	var basepoint6x=Math.random()*4.0/n; var basepoint6y=Math.random()*1.5/n;
...
function render() {
    ...
    for (i=0; i<n; i++) {
       ...
       gl.vertexAttribPointer( myPositions[i], 2, gl.FLOAT, false, 0, 0 );
        gl.enableVertexAttribArray( myPositions[i] );
        xmoves[i]-=8*lightspeed*updateDeltas[i];
		gl.uniform1f( xmoveUniforms[i], xmoves[i] );
	...
	for (i=0; i<n; i++) {
       ...
       gl.vertexAttribPointer( myPositions2[i], 2, gl.FLOAT, false, 0, 0 );
		...
		y=ymovesTriangle[i]+ytransTriangle[i];
		if((Math.abs(clipX-x)<0.03)&&(Math.abs(clipY-y)<0.05))
		{
			lose = true;
		}
			...
			//xmovesTriangle[i]=1;
			//ymovesTriangle[i]=(Math.random()*1);
			if(ymovesTriangle[i] > 0.9){
			random_float = ((Math.random()*15)+75)*0.01;
			}
			else if(ymovesTriangle[i] > 0.69){
			random_float = ((Math.random()*20)+40)*0.01;
			}
			ymovesTriangle[i]=((Math.random()*1)-random_float);
			xmovesTriangle[i]=1;

			//ytransTriangle[i]=(Math.random()*2)-1;
            //updateDeltas[i]=-updateDeltas[i];
        }
        if (i < (count/10)) {
			//gl.vertexAttribPointer(myPositions[i],2,gl.FLOAT,false,0,0);
	...
	if(lose){
		...
		defeat.play();
		setTimeout("location.href = 'project1.html'",5000)
	}
	else if(win){
